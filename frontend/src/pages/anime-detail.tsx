...
import { AnimeProps, CommentaryProps, PartProps } from "@/types"
...
export const AnimeDetail = () => {
    ...
    const { data, isLoading, error, mutate } = useSWR<AnimeProps>(`/anime/${id}`, fetcher)
...
const [wishlistAnimes, setWishlistAnimes] = useState<any[]>([]);
    useEffect(() => {
     ...
     const isInWishlist = (type: "anime", id: string) => {
    if (type === "anime") return wishlistAnimes.some((i) => i._id === id);
    return false;
  };
const handleLike = (type: "anime", item: any) => {
    ...
    const GetAnimesByGenre = useSWR(`/anime/genre/${data?.category}`, fetcher)
    ...
    const handleAddCommentary = async () => {
        ...
        try {
            await Fetch.post("/anime/add-commentary", {
                id: id,
                text: text,
     ...
     const handleAddLike = async () => {
        ...
        try {
            await Fetch.post(`/anime/like/${id}`, { userId: user.id });
            await mutate();
        ...
        <div>
            ...
            <div className="w-full min-h-[110vh] lg:min-h-[60vh]  px-[5%] pt-4 grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-secondary col-span-1 p-4 space-y-2 rounded-md">
                    ...
                    <div className="flex justify-end items-center gap-2 mt-6">
                         <Button onClick={() => handleLike("anime", data)}>
                            {!isInWishlist("anime", data?._id|| "") ? (
                                <Heart />
                            ) : (
                                <HeartCrack />
                            )}
            ...
            <div className="w-full px-[5%]">
                <div className="rounded-md">
                    <h1 className="text-lg bg-secondary p-2 rounded-md font-semibold mb-2">{data?.category} janriga oid animelar</h1>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full min-h-[40vh] p-4 px-[7%]">
                        {GetAnimesByGenre?.data?.map(({ _id, made, title, images }: AnimeProps) => (
                            <div
                                key={_id}
