...
export function AddAnime() {
  ...
  const { mutate } = useSWR("/anime", fetcher)
  const resetForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.title.trim()) newErrors.title = "Anime nomi majburiy.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await Fetch.post("/anime/create", body, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      await mutate()
      toast.success("Anime muvaffaqiyatli qo‘shildi!");

      resetForm();
      setIsSheetOpen(false);
    } catch (error) {
      toast.error("Anime qo‘shishda xatolik yuz berdi.");
      console.error(error);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    ...
    >
      <SheetTrigger asChild>
        <Button variant="default" className="bg-sky-600">Anime qo‘shish</Button>
      </SheetTrigger>
      <SheetContent className="h-full w-full sm:max-w-md sm:h-auto bg-[#202020] text-white border-none">
        <SheetHeader>
          <SheetTitle className="text-white text-2xl">Yangi Anime</SheetTitle>
        </SheetHeader>
