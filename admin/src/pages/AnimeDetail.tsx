...
import { AddAnimePart } from "@/modules/AddAnimePart"
import { AnimeProps, PartProps } from "@/types/RootTypes"
...
export const AnimeDetail = () => {
    const { id } = useParams()
    const { data, isLoading, error, mutate } = useSWR(`/anime/${id}`, fetcher)
    const anime = data as AnimeProps || {}
    const handleDeleteAnimePart = async (partId: string) => {
        try {
            (await Fetch.delete(`anime/${id}/part/${partId}`)).data;
            await mutate()
        ...
        <div className="p-4 h-screen overflow-y-auto">
            ...
            <div className="w-full p-4 grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div className="col-span-1 p-4 space-y-2 rounded-md">
                    ...
                    <h1 className="text-muted-foreground flex flex-wrap items-center justify-between">Tavsif: <span className="text-blue-500 font-semibold">{data?.description}</span></h1>
                    <Separator />
                                                            ...
                                                            <DropdownMenuContent className="border-none space-y-1">
                                                                ...
                                                                <DropdownMenuItem asChild>
                                                                    <Button variant={"destructive"} className="w-full"
                                                                        onClick={() => handleDeleteAnimePart(_id)}
                                                                    >
