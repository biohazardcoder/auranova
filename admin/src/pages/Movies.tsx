...
export default function Movies() {
    const { data: movieData, error, isLoading, mutate } = useSWR("/movie", fetcher)


    const handleDeleteMovie = async (id: string) => {
        try {
            (await Fetch.delete(`movie/${id}`)).data;
            await mutate()
            toast.success("Film muvaffaqiyatli o`chirildi!")
        } catch (error) {
            console.log(error);
        }
    };
    ...
    return (
        <div className="p-4 h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4 gap-3 flex-wrap">
                <h1 className="text-2xl font-bold">Filmlar</h1>
                <Sheet>
                    <AddMovie />
                </Sheet>
            </div>
            ...
            {movieData?.movies?.length <= 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {movieData?.movies?.map(({ _id, images, title, createdAt }: AnimeProps) => (
                        <div
                            key={_id}
